...
class Harness:
    ...
    def create_graph(self) -> pydot.Dot:
        dot = pydot.Dot("graph", graph_type="graph", 
            rankdir="LR",
            ranksep="2",
        ...
        dot.set_edge_defaults(style="bold", fontname=self.options.fontname)
        ...
        for cable in self.cables.values():
            ...
            for connection in cable.connections:
                ...
                if connection.from_pin is not None:  # connect to left
                    ...
                    dot.add_edge(pydot.Edge(code_left_1, code_left_2))
                ...
                if connection.to_pin is not None:  # connect to right
                    ...
                    dot.add_edge(pydot.Edge(code_right_1, code_right_2))
        ...
        for mate in self.mates:
            ...
            dot.set_edge_defaults(color=color, style="dashed", dir=dir)
            dot.add_edge(pydot.Edge(code_from, code_to))
    ...
    def output(
        ...
        for f in fmt:
            if f in ("png", "svg", "html"):
                if f == "html" or f== "svg":  # if HTML format is specified,
                    graph.write_svg(f"{filename}.tmp.svg")
                else:
        ...
        # GraphViz output
        if "gv" in fmt:
            graph.write(path=f"{filename}.gv")
        # BOM output
